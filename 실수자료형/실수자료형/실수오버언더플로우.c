...
int main() {
    ...
    // float의 양수 최솟값을 100000000.0 으로 나누면 아주 작은 수가 되면서 언더플로우 발생
    num1 = num1 / 100000000.0f; 

    //float의 양수 최댓값에 1000.0을 곱하면 저장할 수 있는 범위를 넘어서므로 오버플로우 발생
    num2 = num2 * 1000.0f;

    printf("%e %e\n", num1, num2); // 0.000000e+00 inf: 실수의 언더플로우는 0, 오버플로우는 무한대가 된다.
    // e+00: 10의 거듭제곱을 나타내는 지수 부분, 여기서는 10^0, 즉 1입니다.
}